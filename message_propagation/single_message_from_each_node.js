...
try {
    ...
    console.log('error starting the node', e)
    process.exit(1)
}

node.networkNode.subscribe(group)

node.networkNode.addGroupMessageHandler(group, (e) => {
...
if (!opts.seed.startsWith("bootstrap")) {
    const index = parseInt(opts.seed.split("node")[1])
    console.log(`Node ${index} sending message`)
    ...
    node.networkNode.broadcastMessage(group, {
        ...
        type: 0,
        data: new Uint8Array(Buffer.from(`Message from ${node.networkNode.peerId}`))
    })
}
